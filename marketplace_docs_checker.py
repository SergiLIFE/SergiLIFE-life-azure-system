...
class MarketplaceDocumentationChecker:
    def __init__(self):
        ...
        self.required_docs = {
            ...
            "azure.yaml": {
                "required": True,
                "description": "Azure deployment config",
                "min_size": 200,
            },
            "requirements.txt": {
                "required": True,
    ...
    def run_comprehensive_check(self) -> dict:
        ...
        results = {
            ...
            "summary": {
                ...
                "required_files": len(
                    [k for k, v in self.required_docs.items() if v["required"]]
                ),
                "optional_files": len(
                    [k for k, v in self.required_docs.items() if not v["required"]]
                ),
                "found_files": 0,
                "missing_required": 0,
                "missing_optional": 0,
        ...
        for file_path, requirements in self.required_docs.items():
            ...
            if file_path in self.marketplace_content_requirements:
                content_check = self.check_content_requirements(
                    file_path, self.marketplace_content_requirements[file_path]
                )
        ...
        required_found = (
            results["summary"]["required_files"]
        ...
        optional_score = (
            (optional_found / results["summary"]["optional_files"]) * 20
            if results["summary"]["optional_files"] > 0
            else 0
        )

        results["summary"]["readiness_score"] = required_score + optional_score
        results["summary"]["marketplace_ready"] = (
            results["summary"]["readiness_score"] >= 75
        )
    ...
    def print_detailed_report(self, results: dict):
        ...
        optional_files = {
            k: v for k, v in results["files"].items() if not v["required"]
        }
