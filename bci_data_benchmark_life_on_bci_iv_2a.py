...
def compare_to_sota_and_best(aggregate, sota_acc=0.8515, sota_latency=15.12, best_acc=None, best_latency=None):
    ...
    best_str = (
        f"Best: {best_acc*100:.2f}%" if best_acc is not None else "Best: N/A"
    )
    ...
    compliance = (
        "PASS" if aggregate['mean_latency_ms'] < sota_latency else "FAIL"
    )
