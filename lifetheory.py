...
class AdaptationParameters:
    ...
    # Compatibility aliases for newer modules
    @property
    def decay_factor(self) -> float:
        """Alias for experience_weight_decay"""
        return self.experience_weight_decay

    @decay_factor.setter
    def decay_factor(self, value: float):
        """Setter for decay_factor alias"""
        self.experience_weight_decay = value
...
class ExperienceMemory:
    ...
    def get_best_experiences(self, count: int = 10) -> List[LearningExperience]:
        ...
        sorted_experiences = sorted(
            self.experiences, key=lambda x: x.performance_metric, reverse=True
        )
...
class VenturiGateSystem:
    ...
    def adapt_gates(self, performance_feedback: float) -> None:
        ...
        self.gate_states = np.clip(self.gate_states + adaptation, 0.1, 2.0)
...
class CoreLIFEAlgorithm(LIFEProcessor):
    ...
    def _apply_experience_adaptation(self, data: np.ndarray) -> np.ndarray:
        ...
        adaptation_weights = np.array(
            [exp.performance_metric for exp in best_experiences]
        )
    ...
    def _individual_learning_process(
        self, data: np.ndarray, context: Dict[str, Any]
    ) -> np.ndarray:
        ...
        adaptation_factor = 1.0 + self.params.learning_rate * np.random.normal(
            0, 0.1, data.shape
        )
    ...
    def adapt(self, experience: LearningExperience) -> None:
        ...
        logger.info(
            f"L.I.F.E adapted with performance: {experience.performance_metric}"
        )
    ...
    def _update_adaptation_matrix(self, experience: LearningExperience) -> None:
        ...
        adaptation_update = (
            np.eye(self.adaptation_matrix.shape[0]) * learning_adjustment
        )
    ...
    def _update_performance_baseline(self) -> None:
        ...
        if len(self.memory.performance_history) >= 10:
            ...
            self.performance_baseline = (
                0.9 * self.performance_baseline + 0.1 * recent_performance
            )
    ...
    def get_performance_metrics(self) -> Dict[str, float]:
        ...
        return {
            ...
            "recent_performance": (
                np.mean(self.memory.performance_history[-10:])
                if len(self.memory.performance_history) >= 10
                else 0.0
            ),
    ...
    def save_state(self, filepath: str) -> None:
        ...
        state = {
            ...
            "experience_count": len(self.memory.experiences),
            "performance_history": self.memory.performance_history[
                -100:
            ],  # Save last 100
        }
...
class LIFEEEGProcessor(CoreLIFEAlgorithm):
    ...
    def process_eeg(
        self, eeg_data: np.ndarray, channels: List[str] = None
    ) -> Dict[str, np.ndarray]:
        ...
        for i, channel_data in enumerate(eeg_data):
            ...
            processed = self.process(
                channel_data, {"channel": channel_name, "type": "eeg"}
            )
