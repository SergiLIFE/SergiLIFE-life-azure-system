...
Visualize L.I.F.E. algorithm performance benchmarks
...
def plot_benchmark_results(aggregate_file=AGGREGATE_FILE):
    ...
    plt.axhline(
        data["mean_accuracy"] * 100, color="g", linestyle="--", label="Mean Accuracy"
    )
    plt.xlabel("Run")
    plt.ylabel("Accuracy (%)")
    ...
    plt.axhline(
        data["mean_latency_ms"], color="r", linestyle="--", label="Mean Latency"
    )
    plt.xlabel("Run")
